...
Aggiornato al 2026-09-09. Contiene la sezione Database (entità `t_task`/`t_project`/`t_milestone`/`t_state`, convenzioni, vincoli, migrazione), la sezione Flussi (macchina a stati: nascita, reminder, cascata di chiusura/riapertura, migrazione esterna) e la sezione Interfaccia (schermata principale: colonna Inbox ridimensionabile/magnetica, ambito progetto, colonna contenuto con selettore vista). Resta volutamente aperta una sola questione di modello dati, rimandata: i dettagli dell'oggetto sorgente esterno (gli "stati speciali" del task sono stati chiusi il 2026-09-10 con il flag `isInbox`, da implementare) — più la progettazione della schermata "Full Inbox" lato interfaccia (vedi sezione Interfaccia).
...
3. ~~**Stati speciali del task (inbox/"non preparato")**~~ **deciso (2026-09-10)**: flag dedicato `isInbox` su `t_task`, non una condizione derivata da progetto/date — un task in inbox deve poter essere messo in agenda restando da triare (vedi nota nella sezione Entità t_task). Da implementare: colonna, default per sorgente, gesto di uscita, migrazione.
...
1. **Le origini da confermare non hanno un dato dietro.** Nel core non esiste un flag "non ancora confermata" — era la domanda che la sezione Database lasciava aperta sotto "Stati speciali del task", ora chiusa (2026-09-10): il dato sarà il flag `isInbox`, e la colonna dovrà leggere quello. Finché la colonna non esiste, la vista legge ciò che il modello sa già dire: task nata da una sorgente esterna (`sourceType <> 'manual'`) e ancora nello stato di partenza; "confermare" la porta sul primo stato intermedio. Se non ce n'è uno configurato — è il caso del database attuale, che ha solo `Da fare` e `Fatto` — il gesto è spento e il pulsante spiega perché. Da rifare su `isInbox` quando il campo viene implementato.
