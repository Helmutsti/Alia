...
export default function SourcesScreen() {
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, maxWidth: 800 }}>
      ...
      <p style={{ fontSize: "var(--text-md)", lineHeight: 1.55, color: "color-mix(in srgb, var(--color-text) 66%, transparent)", margin: "0 0 20px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20, overflowY: "auto", minHeight: 0, paddingRight: 6, opacity: 0.85 }}>
        {GROUPS.map((g) => {
          ...
          return (
            <div key={g.key}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, padding: "0 12px" }}>
                <span style={{ fontSize: "var(--text-sm)", letterSpacing: "0.12em", textTransform: "uppercase", color: g.color }}>{g.label}</span>
                <span style={{ fontSize: "var(--text-sm)", color: "color-mix(in srgb, var(--color-text) 55%, transparent)" }}>{items.length}</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {pending.map((item) => (
                  <div key={item.proposal} className="card" style={{ gap: 0, padding: 12, borderLeft: "2px solid var(--color-accent)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "var(--text-sm)", color: "color-mix(in srgb, var(--color-text) 57%, transparent)" }}>
                      {SEND_ICON[item.kind]}
                    ...
                    <div style={{ fontSize: "var(--text-md)", lineHeight: 1.5, marginTop: 8, color: "color-mix(in srgb, var(--color-text) 80%, transparent)" }}>{item.excerpt}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12, paddingTop: 11, borderTop: "1px solid var(--color-divider)", flexWrap: "wrap" }}>
                      <span style={{ width: 8, height: 8, flex: "0 0 auto", rotate: "45deg", background: "var(--color-accent)" }} />
                      <span style={{ fontFamily: "var(--font-heading)", fontWeight: 500, fontSize: "var(--text-md)", letterSpacing: "-0.01em" }}>{item.proposal}</span>
                      <span style={{ display: "flex", alignItems: "center", gap: 4, marginLeft: "auto" }}>
                        {item.chips.map((c) => (
                          <span key={c} style={{ display: "inline-flex", alignItems: "center", height: 23, padding: "0 8px", fontSize: "var(--text-sm)", borderRadius: "var(--radius-md)", border: "1px solid var(--color-divider)", color: "color-mix(in srgb, var(--color-text) 74%, transparent)" }}>{c}</span>
                        ))}
                        <button type="button" className="ghost-send" disabled title="Crea il task" aria-label="Crea il task" style={{ display: "grid", placeItems: "center", width: 27, height: 27, padding: 0, border: "none", borderRadius: "var(--radius-md)", background: "transparent", cursor: "not-allowed", color: "var(--color-accent)", marginLeft: 4 }}>
                          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M21.5 2.5L2.8 9.6c-.7.3-.7 1.2 0 1.5l7.1 2.7c.2.1.4.3.5.5l2.7 7.1c.3.7 1.2.7 1.5 0z" /><path d="M10.4 13.6l5.6-5.6" /></svg>
                        </button>
                ...
                {integrated.map((item) => (
                  <div key={item.proposal} style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px", borderBottom: "1px solid var(--color-divider)", fontSize: "var(--text-base-sm)" }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ flex: "0 0 auto" }}><polyline points="20 6 9 17 4 12" /></svg>
                    <span style={{ fontSize: "var(--text-sm)", color: "color-mix(in srgb, var(--color-text) 57%, transparent)", whiteSpace: "nowrap" }}>{item.source}</span>
                    ...
                    <span style={{ marginLeft: "auto", flex: "0 0 auto", fontSize: "var(--text-sm)", color: "var(--color-accent-300)" }}>{item.landed}</span>
                  </div>
                ))}
              </div>
            </div>
