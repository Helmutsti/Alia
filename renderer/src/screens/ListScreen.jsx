    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      <div style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--color-accent)", marginBottom: 8 }}>
          {isToday ? formatWeekdayLong() : "Archivio"}
        </div>
        <h2 style={{ margin: 0, fontSize: 38 }}>{isToday ? "Oggi" : "Tutti i task"}</h2>
      </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowX: "hidden", overflowY: "auto" }}>
        {view === "kanban" ? (
          ...
          <div style={{ marginTop: 14 }}>
            {dim === "none" && (
              <div style={{ display: "grid", gridTemplateColumns: "14px minmax(0,1fr) 110px 90px 72px", gap: 11, padding: "0 12px 8px", fontSize: 10, letterSpacing: "0.12em", textTransform: "uppercase", color: "color-mix(in srgb, var(--color-text) 57%, transparent)", borderBottom: "1px solid var(--color-divider)" }}>
                <span />
                <span>Task</span>
                <span>Progetto</span>
                <span>Scadenza</span>
                  ...
                  {dim !== "none" && (
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, padding: "0 12px" }}>
                      <button
                        ...
                        style={{ display: "inline-flex", alignItems: "center", gap: 7, border: "none", background: "transparent", cursor: "pointer", padding: 0, color: g.color, fontFamily: "var(--font-body)" }}
                      >
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round" style={{ transform: collapsed[g.key] ? "rotate(0deg)" : "rotate(90deg)", opacity: 0.6 }}>
                          <path d="M9 5l7 7-7 7" />
                        </svg>
                        <span style={{ fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase" }}>{g.label}</span>
                      </button>
                      <span style={{ fontSize: 11, color: "color-mix(in srgb, var(--color-text) 55%, transparent)" }}>{g.rows.length}</span>
            ...
            {sorted.length === 0 && (
              <div style={{ padding: "26px 12px", fontSize: 12.5, color: "color-mix(in srgb, var(--color-text) 58%, transparent)" }}>
                {isToday ? (
